  ...
  Future<void> _submit() async {
    ...
    if (success && mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Perfil de conductor completado. Tu solicitud está pendiente de aprobación del administrador.',
          ),
  ...
  Widget _buildResubmitButton(BuildContext context, AuthProvider authProvider) {
    return SizedBox(
      ...
      child: OutlinedButton.icon(
            ...
            authProvider.isLoading
                ...
                : () async {
                  ...
                  if (confirmed == true && mounted) {
                    final success =
                        await authProvider.resubmitDriverApplication();
                    if (success && mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text(
                            'Solicitud reenviada exitosamente. Está pendiente de revisión.',
                          ),
                          backgroundColor: Colors.orange,
                          duration: Duration(seconds: 4),
                        ),
        ...
        label: Text(
          authProvider.isLoading ? 'Reenviando...' : 'Reenviar Solicitud',
        ),
            ...
            children: [
              ...
              authProvider.isLoading
                  ...
                  : ElevatedButton.icon(
                    onPressed: _submit,
                    icon: Icon(
                      isAlreadyDriver ? Icons.save : Icons.check_circle,
                    ),
                    label: Text(
                      isAlreadyDriver
                          ? 'ACTUALIZAR DATOS DEL VEHÍCULO'
