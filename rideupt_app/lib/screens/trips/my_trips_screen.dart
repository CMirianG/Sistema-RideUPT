      ...
      body: SafeAreaWrapper(
        child: Consumer<TripProvider>(
          builder: (context, tripProvider, child) {
            ...
            // Historial: SOLO viajes completados donde el usuario participó
            // IMPORTANTE: Los viajes completados NO deben verificar expiración por tiempo,
            // solo verificar que el estado sea 'completado' (no 'expirado')
            // Ordenar por fecha de creación (más recientes primero) y tomar solo los últimos 10
            final historyTrips = tripProvider.myTrips
                .where((trip) {
                  // Verificar que el viaje esté completado usando el getter isCompleted
                  // que verifica tanto 'completado' como 'completed'
                  final isCompleted = trip.isCompleted;
                  
                  // Verificar que NO esté cancelado, en proceso, o con estado 'expirado'
                  // NO verificar hasTimeExpired porque los completados pueden tener expiresAt pasado
                  ...
                  if (currentUserId != null) {
                    if (isDriver) {
                      // Para conductores: verificar que el conductor sea el dueño del viaje
                      userParticipated = trip.driver.id == currentUserId;
                    } else {
                      // Para pasajeros: verificar que el usuario esté en la lista de pasajeros confirmados
                      userParticipated = trip.passengers.any(
                        (p) => p.user.id == currentUserId && p.status == 'confirmed'
                      );
                    }
              ...
              ..sort((a, b) {
                ...
                // Si no hay createdAt, usar departureTime
                return b.departureTime.compareTo(a.departureTime);
              });
            
            final limitedHistoryTrips = historyTrips.take(10).toList();
            ...
            return RefreshIndicator(
              onRefresh: () => tripProvider.fetchMyTrips(force: true),
